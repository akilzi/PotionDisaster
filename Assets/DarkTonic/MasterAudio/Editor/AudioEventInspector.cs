...
public class AudioEventInspector : Editor {
    ...
    public override void OnInspectorGUI() {
        ...
        #if UNITY_4_6 || UNITY_5_0
            var showNewUIEvents = sounds.unityUIMode == EventSounds.UnityUIVersion.uGUI;
			var hasSlider = sounds.GetComponent<Slider>() != null;
			var hasButton = sounds.GetComponent<Button>() != null;
			var hasRect = sounds.GetComponent<RectTransform>() != null;
		...
		if (sounds.unityUIMode == EventSounds.UnityUIVersion.Legacy) {
        ...
        if (!sounds.disableSounds) {
			...
			var newUI = (EventSounds.UnityUIVersion) EditorGUILayout.EnumPopup("Unity UI Version", sounds.unityUIMode);
			if (newUI != sounds.unityUIMode) {
				UndoHelper.RecordObjectPropertyForUndo(ref isDirty, sounds, "change Unity UI Version");
				sounds.unityUIMode = newUI;
			}
        ...
        if (sounds.useMouseEnterSound) {
            ...
            EditorGUILayout.BeginHorizontal(EditorStyles.objectFieldThumb);
            var newMouseEnter = EditorGUILayout.Toggle("Mouse Enter (Legacy)" + DisabledText, sounds.useMouseEnterSound);
        ...
        if (sounds.useMouseExitSound) {
            ...
            EditorGUILayout.BeginHorizontal(EditorStyles.objectFieldThumb);
			var newMouseEnter = EditorGUILayout.Toggle("Mouse Exit (Legacy)" + DisabledText, sounds.useMouseExitSound);
        ...
        if (sounds.useMouseClickSound) {
            ...
            EditorGUILayout.BeginHorizontal(EditorStyles.objectFieldThumb);
			var newMouseClick = EditorGUILayout.Toggle("Mouse Down (Legacy)" + DisabledText, sounds.useMouseClickSound);
        ...
        if (sounds.useMouseDragSound) {
            ...
            EditorGUILayout.BeginHorizontal(EditorStyles.objectFieldThumb);
			var newMouseClick = EditorGUILayout.Toggle("Mouse Drag (Legacy)" + DisabledText, sounds.useMouseDragSound);
        ...
        if (sounds.useMouseUpSound) {
            ...
            EditorGUILayout.BeginHorizontal(EditorStyles.objectFieldThumb);
			var newMouseClick = EditorGUILayout.Toggle("Mouse Up (Legacy)" + DisabledText, sounds.useMouseUpSound);
        ...
        if (showNewUIEvents) {
			if (hasSlider) {
				if (sounds.useUnitySliderChangedSound) {
					EditorGUI.indentLevel = 0;
					GUI.color = sounds.useUnitySliderChangedSound ? MasterAudioInspector.activeClr : MasterAudioInspector.inactiveClr;
					EditorGUILayout.BeginHorizontal(EditorStyles.objectFieldThumb);
					var newMouseClick = EditorGUILayout.Toggle("Slider Changed (uGUI)" + DisabledText, sounds.useUnitySliderChangedSound);
					if (newMouseClick != sounds.useUnitySliderChangedSound) {
						if (newMouseClick) {
							ActivateEvent(EventSounds.EventType.UnitySliderChanged);
						} else {
					...
					EditorGUILayout.EndHorizontal();
					GUI.color = Color.white;
					if (sounds.useUnitySliderChangedSound && !sounds.disableSounds) {
						UndoHelper.RecordObjectPropertyForUndo(ref isDirty, sounds, "toggle Slider Changed (uGUI)");
						changedList.Add(RenderAudioEvent(sounds.unitySliderChangedSound, EventSounds.EventType.UnitySliderChanged));
					}
				}
			}
			...
			if (hasButton) {
				if (sounds.useUnityButtonClickedSound) {
					EditorGUI.indentLevel = 0;
					GUI.color = sounds.useUnityButtonClickedSound ? MasterAudioInspector.activeClr : MasterAudioInspector.inactiveClr;
					EditorGUILayout.BeginHorizontal(EditorStyles.objectFieldThumb);
					var newMouseClick = EditorGUILayout.Toggle("Button Click (uGUI)" + DisabledText, sounds.useUnityButtonClickedSound);
					if (newMouseClick != sounds.useUnityButtonClickedSound) {
						if (newMouseClick) {
							ActivateEvent(EventSounds.EventType.UnityButtonClicked);
						} else {
					...
					EditorGUILayout.EndHorizontal();
					GUI.color = Color.white;
					if (sounds.useUnityButtonClickedSound && !sounds.disableSounds) {
						UndoHelper.RecordObjectPropertyForUndo(ref isDirty, sounds, "toggle Button Click (uGUI)");
						changedList.Add(RenderAudioEvent(sounds.unityButtonClickedSound, EventSounds.EventType.UnityButtonClicked));
					}
				}
			}
			...
			if (canClick) {
				if (sounds.useUnityPointerEnterSound) {
					EditorGUI.indentLevel = 0;
					GUI.color = sounds.useUnityPointerEnterSound ? MasterAudioInspector.activeClr : MasterAudioInspector.inactiveClr;
					EditorGUILayout.BeginHorizontal(EditorStyles.objectFieldThumb);
					var newMouseClick = EditorGUILayout.Toggle("Pointer Enter (uGUI)" + DisabledText, sounds.useUnityPointerEnterSound);
					if (newMouseClick != sounds.useUnityPointerEnterSound) {
						if (newMouseClick) {
							ActivateEvent(EventSounds.EventType.UnityPointerEnter);
						} else {
					...
					EditorGUILayout.EndHorizontal();
					GUI.color = Color.white;
					if (sounds.useUnityPointerEnterSound && !sounds.disableSounds) {
						UndoHelper.RecordObjectPropertyForUndo(ref isDirty, sounds, "toggle Pointer Enter (uGUI)");
						changedList.Add(RenderAudioEvent(sounds.unityPointerEnterSound, EventSounds.EventType.UnityPointerEnter));
					}
				}

				if (sounds.useUnityPointerExitSound) {
					EditorGUI.indentLevel = 0;
					GUI.color = sounds.useUnityPointerExitSound ? MasterAudioInspector.activeClr : MasterAudioInspector.inactiveClr;
					EditorGUILayout.BeginHorizontal(EditorStyles.objectFieldThumb);
					var newMouseClick = EditorGUILayout.Toggle("Pointer Exit (uGUI)" + DisabledText, sounds.useUnityPointerExitSound);
					if (newMouseClick != sounds.useUnityPointerExitSound) {
						if (newMouseClick) {
							ActivateEvent(EventSounds.EventType.UnityPointerExit);
						} else {
					...
					EditorGUILayout.EndHorizontal();
					GUI.color = Color.white;
					if (sounds.useUnityPointerExitSound && !sounds.disableSounds) {
						UndoHelper.RecordObjectPropertyForUndo(ref isDirty, sounds, "toggle Pointer Exit (uGUI)");
						changedList.Add(RenderAudioEvent(sounds.unityPointerExitSound, EventSounds.EventType.UnityPointerExit));
					}
				}

				if (sounds.useUnityPointerDownSound) {
					EditorGUI.indentLevel = 0;
					GUI.color = sounds.useUnityPointerDownSound ? MasterAudioInspector.activeClr : MasterAudioInspector.inactiveClr;
					EditorGUILayout.BeginHorizontal(EditorStyles.objectFieldThumb);
					var newMouseClick = EditorGUILayout.Toggle("Pointer Down (uGUI)" + DisabledText, sounds.useUnityPointerDownSound);
					if (newMouseClick != sounds.useUnityPointerDownSound) {
						if (newMouseClick) {
							ActivateEvent(EventSounds.EventType.UnityPointerDown);
						} else {
					...
					EditorGUILayout.EndHorizontal();
					GUI.color = Color.white;
					if (sounds.useUnityPointerDownSound && !sounds.disableSounds) {
						UndoHelper.RecordObjectPropertyForUndo(ref isDirty, sounds, "toggle Pointer Down (uGUI)");
						changedList.Add(RenderAudioEvent(sounds.unityPointerDownSound, EventSounds.EventType.UnityPointerDown));
					}
				}

				if (sounds.useUnityPointerUpSound) {
					EditorGUI.indentLevel = 0;
					GUI.color = sounds.useUnityPointerUpSound ? MasterAudioInspector.activeClr : MasterAudioInspector.inactiveClr;
					EditorGUILayout.BeginHorizontal(EditorStyles.objectFieldThumb);
					var newMouseClick = EditorGUILayout.Toggle("Pointer Up (uGUI)" + DisabledText, sounds.useUnityPointerUpSound);
					if (newMouseClick != sounds.useUnityPointerUpSound) {
						if (newMouseClick) {
							ActivateEvent(EventSounds.EventType.UnityPointerUp);
						} else {
					...
					EditorGUILayout.EndHorizontal();
					GUI.color = Color.white;
					if (sounds.useUnityPointerUpSound && !sounds.disableSounds) {
						UndoHelper.RecordObjectPropertyForUndo(ref isDirty, sounds, "toggle Pointer Up (uGUI)");
						changedList.Add(RenderAudioEvent(sounds.unityPointerUpSound, EventSounds.EventType.UnityPointerUp));
					}
				}

				if (sounds.useUnityDragSound) {
					EditorGUI.indentLevel = 0;
					GUI.color = sounds.useUnityDragSound ? MasterAudioInspector.activeClr : MasterAudioInspector.inactiveClr;
					EditorGUILayout.BeginHorizontal(EditorStyles.objectFieldThumb);
					var newMouseClick = EditorGUILayout.Toggle("Drag (uGUI)" + DisabledText, sounds.useUnityDragSound);
					if (newMouseClick != sounds.useUnityDragSound) {
						if (newMouseClick) {
							ActivateEvent(EventSounds.EventType.UnityDrag);
						} else {
					...
					EditorGUILayout.EndHorizontal();
					GUI.color = Color.white;
					if (sounds.useUnityDragSound && !sounds.disableSounds) {
						UndoHelper.RecordObjectPropertyForUndo(ref isDirty, sounds, "toggle Drag (uGUI)");
						changedList.Add(RenderAudioEvent(sounds.unityDragSound, EventSounds.EventType.UnityDrag));
					}
				}

				if (sounds.useUnityDropSound) {
					EditorGUI.indentLevel = 0;
					GUI.color = sounds.useUnityDropSound ? MasterAudioInspector.activeClr : MasterAudioInspector.inactiveClr;
					EditorGUILayout.BeginHorizontal(EditorStyles.objectFieldThumb);
					var newMouseClick = EditorGUILayout.Toggle("Drop (uGUI)" + DisabledText, sounds.useUnityDropSound);
					if (newMouseClick != sounds.useUnityDropSound) {
						if (newMouseClick) {
							ActivateEvent(EventSounds.EventType.UnityDrop);
						} else {
					...
					EditorGUILayout.EndHorizontal();
					GUI.color = Color.white;
					if (sounds.useUnityDropSound && !sounds.disableSounds) {
						UndoHelper.RecordObjectPropertyForUndo(ref isDirty, sounds, "toggle Drop (uGUI)");
						changedList.Add(RenderAudioEvent(sounds.unityDropSound, EventSounds.EventType.UnityDrop));
					}
				}

				if (sounds.useUnityScrollSound) {
					EditorGUI.indentLevel = 0;
					GUI.color = sounds.useUnityScrollSound ? MasterAudioInspector.activeClr : MasterAudioInspector.inactiveClr;
					EditorGUILayout.BeginHorizontal(EditorStyles.objectFieldThumb);
					var newMouseClick = EditorGUILayout.Toggle("Scroll (uGUI)" + DisabledText, sounds.useUnityScrollSound);
					if (newMouseClick != sounds.useUnityScrollSound) {
						if (newMouseClick) {
							ActivateEvent(EventSounds.EventType.UnityScroll);
						} else {
					...
					EditorGUILayout.EndHorizontal();
					GUI.color = Color.white;
					if (sounds.useUnityScrollSound && !sounds.disableSounds) {
						UndoHelper.RecordObjectPropertyForUndo(ref isDirty, sounds, "toggle Scroll (uGUI)");
						changedList.Add(RenderAudioEvent(sounds.unityScrollSound, EventSounds.EventType.UnityScroll));
					}
				}

				if (sounds.useUnityUpdateSelectedSound) {
					EditorGUI.indentLevel = 0;
					GUI.color = sounds.useUnityUpdateSelectedSound ? MasterAudioInspector.activeClr : MasterAudioInspector.inactiveClr;
					EditorGUILayout.BeginHorizontal(EditorStyles.objectFieldThumb);
					var newMouseClick = EditorGUILayout.Toggle("Update Selected (uGUI)" + DisabledText, sounds.useUnityUpdateSelectedSound);
					if (newMouseClick != sounds.useUnityUpdateSelectedSound) {
						if (newMouseClick) {
							ActivateEvent(EventSounds.EventType.UnityUpdateSelected);
						} else {
					...
					EditorGUILayout.EndHorizontal();
					GUI.color = Color.white;
					if (sounds.useUnityUpdateSelectedSound && !sounds.disableSounds) {
						UndoHelper.RecordObjectPropertyForUndo(ref isDirty, sounds, "toggle Update Selected (uGUI)");
						changedList.Add(RenderAudioEvent(sounds.unityUpdateSelectedSound, EventSounds.EventType.UnityUpdateSelected));
					}
				}

				if (sounds.useUnitySelectSound) {
					EditorGUI.indentLevel = 0;
					GUI.color = sounds.useUnitySelectSound ? MasterAudioInspector.activeClr : MasterAudioInspector.inactiveClr;
					EditorGUILayout.BeginHorizontal(EditorStyles.objectFieldThumb);
					var newMouseClick = EditorGUILayout.Toggle("Select (uGUI)" + DisabledText, sounds.useUnitySelectSound);
					if (newMouseClick != sounds.useUnitySelectSound) {
						if (newMouseClick) {
							ActivateEvent(EventSounds.EventType.UnitySelect);
						} else {
					...
					EditorGUILayout.EndHorizontal();
					GUI.color = Color.white;
					if (sounds.useUnitySelectSound && !sounds.disableSounds) {
						UndoHelper.RecordObjectPropertyForUndo(ref isDirty, sounds, "toggle Select (uGUI)");
						changedList.Add(RenderAudioEvent(sounds.unitySelectSound, EventSounds.EventType.UnitySelect));
					}
				}

				if (sounds.useUnityDeselectSound) {
					EditorGUI.indentLevel = 0;
					GUI.color = sounds.useUnityDeselectSound ? MasterAudioInspector.activeClr : MasterAudioInspector.inactiveClr;
					EditorGUILayout.BeginHorizontal(EditorStyles.objectFieldThumb);
					var newMouseClick = EditorGUILayout.Toggle("Deselect (uGUI)" + DisabledText, sounds.useUnityDeselectSound);
					if (newMouseClick != sounds.useUnityDeselectSound) {
						if (newMouseClick) {
							ActivateEvent(EventSounds.EventType.UnityDeselect);
						} else {
					...
					EditorGUILayout.EndHorizontal();
					GUI.color = Color.white;
					if (sounds.useUnityDeselectSound && !sounds.disableSounds) {
						UndoHelper.RecordObjectPropertyForUndo(ref isDirty, sounds, "toggle Deselect (uGUI)");
						changedList.Add(RenderAudioEvent(sounds.unityDeselectSound, EventSounds.EventType.UnityDeselect));
					}
				}

				if (sounds.useUnityMoveSound) {
					EditorGUI.indentLevel = 0;
					GUI.color = sounds.useUnityMoveSound ? MasterAudioInspector.activeClr : MasterAudioInspector.inactiveClr;
					EditorGUILayout.BeginHorizontal(EditorStyles.objectFieldThumb);
					var newMouseClick = EditorGUILayout.Toggle("Move (uGUI)" + DisabledText, sounds.useUnityMoveSound);
					if (newMouseClick != sounds.useUnityMoveSound) {
						if (newMouseClick) {
							ActivateEvent(EventSounds.EventType.UnityMove);
						} else {
					...
					EditorGUILayout.EndHorizontal();
					GUI.color = Color.white;
					if (sounds.useUnityMoveSound && !sounds.disableSounds) {
						UndoHelper.RecordObjectPropertyForUndo(ref isDirty, sounds, "toggle Move (uGUI)");
						changedList.Add(RenderAudioEvent(sounds.unityMoveSound, EventSounds.EventType.UnityMove));
					}
				}

				if (sounds.useUnityInitializePotentialDragSound) {
					EditorGUI.indentLevel = 0;
					GUI.color = sounds.useUnityInitializePotentialDragSound ? MasterAudioInspector.activeClr : MasterAudioInspector.inactiveClr;
					EditorGUILayout.BeginHorizontal(EditorStyles.objectFieldThumb);
					var newMouseClick = EditorGUILayout.Toggle("Initialize Potential Drag (uGUI)" + DisabledText, sounds.useUnityInitializePotentialDragSound);
					if (newMouseClick != sounds.useUnityInitializePotentialDragSound) {
						if (newMouseClick) {
							ActivateEvent(EventSounds.EventType.UnityInitializePotentialDrag);
						} else {
					...
					EditorGUILayout.EndHorizontal();
					GUI.color = Color.white;
					if (sounds.useUnityInitializePotentialDragSound && !sounds.disableSounds) {
						UndoHelper.RecordObjectPropertyForUndo(ref isDirty, sounds, "toggle Initialize Potential Drag (uGUI)");
						changedList.Add(RenderAudioEvent(sounds.unityInitializePotentialDragSound, EventSounds.EventType.UnityInitializePotentialDrag));
					}
				}

				if (sounds.useUnityBeginDragSound) {
					EditorGUI.indentLevel = 0;
					GUI.color = sounds.useUnityBeginDragSound ? MasterAudioInspector.activeClr : MasterAudioInspector.inactiveClr;
					EditorGUILayout.BeginHorizontal(EditorStyles.objectFieldThumb);
					var newMouseClick = EditorGUILayout.Toggle("Begin Drag (uGUI)" + DisabledText, sounds.useUnityBeginDragSound);
					if (newMouseClick != sounds.useUnityBeginDragSound) {
						if (newMouseClick) {
							ActivateEvent(EventSounds.EventType.UnityBeginDrag);
						} else {
					...
					EditorGUILayout.EndHorizontal();
					GUI.color = Color.white;
					if (sounds.useUnityBeginDragSound && !sounds.disableSounds) {
						UndoHelper.RecordObjectPropertyForUndo(ref isDirty, sounds, "toggle Begin Drag (uGUI)");
						changedList.Add(RenderAudioEvent(sounds.unityBeginDragSound, EventSounds.EventType.UnityBeginDrag));
					}
				}

				if (sounds.useUnityEndDragSound) {
					EditorGUI.indentLevel = 0;
					GUI.color = sounds.useUnityEndDragSound ? MasterAudioInspector.activeClr : MasterAudioInspector.inactiveClr;
					EditorGUILayout.BeginHorizontal(EditorStyles.objectFieldThumb);
					var newMouseClick = EditorGUILayout.Toggle("End Drag (uGUI)" + DisabledText, sounds.useUnityEndDragSound);
					if (newMouseClick != sounds.useUnityEndDragSound) {
						if (newMouseClick) {
							ActivateEvent(EventSounds.EventType.UnityEndDrag);
						} else {
					...
					EditorGUILayout.EndHorizontal();
					GUI.color = Color.white;
					if (sounds.useUnityEndDragSound && !sounds.disableSounds) {
						UndoHelper.RecordObjectPropertyForUndo(ref isDirty, sounds, "toggle End Drag (uGUI)");
						changedList.Add(RenderAudioEvent(sounds.unityEndDragSound, EventSounds.EventType.UnityEndDrag));
					}
				}

				if (sounds.useUnitySubmitSound) {
					EditorGUI.indentLevel = 0;
					GUI.color = sounds.useUnitySubmitSound ? MasterAudioInspector.activeClr : MasterAudioInspector.inactiveClr;
					EditorGUILayout.BeginHorizontal(EditorStyles.objectFieldThumb);
					var newMouseClick = EditorGUILayout.Toggle("Submit (uGUI)" + DisabledText, sounds.useUnitySubmitSound);
					if (newMouseClick != sounds.useUnitySubmitSound) {
						if (newMouseClick) {
							ActivateEvent(EventSounds.EventType.UnitySubmit);
						} else {
					...
					EditorGUILayout.EndHorizontal();
					GUI.color = Color.white;
					if (sounds.useUnitySubmitSound && !sounds.disableSounds) {
						UndoHelper.RecordObjectPropertyForUndo(ref isDirty, sounds, "toggle Submit (uGUI)");
						changedList.Add(RenderAudioEvent(sounds.unitySubmitSound, EventSounds.EventType.UnitySubmit));
					}
				}

				if (sounds.useUnityCancelSound) {
					EditorGUI.indentLevel = 0;
					GUI.color = sounds.useUnityCancelSound ? MasterAudioInspector.activeClr : MasterAudioInspector.inactiveClr;
					EditorGUILayout.BeginHorizontal(EditorStyles.objectFieldThumb);
					var newMouseClick = EditorGUILayout.Toggle("Cancel (uGUI)" + DisabledText, sounds.useUnityCancelSound);
					if (newMouseClick != sounds.useUnityCancelSound) {
						if (newMouseClick) {
							ActivateEvent(EventSounds.EventType.UnityCancel);
						} else {
					...
					EditorGUILayout.EndHorizontal();
					GUI.color = Color.white;
					if (sounds.useUnityCancelSound && !sounds.disableSounds) {
						UndoHelper.RecordObjectPropertyForUndo(ref isDirty, sounds, "toggle Cancel (uGUI)");
						changedList.Add(RenderAudioEvent(sounds.unityCancelSound, EventSounds.EventType.UnityCancel));
					}
				}
			}
		}
        ...
        if (sounds.showNGUI) {
            ...
            if (sounds.useNguiMouseDownSound) {
                EditorGUI.indentLevel = 0;
            ...
            if (sounds.useNguiMouseEnterSound) {
                EditorGUI.indentLevel = 0;
            ...
            if (sounds.useNguiMouseExitSound) {
                EditorGUI.indentLevel = 0;
    ...
    private bool HasEventTrigger {
		get {
			return sounds.GetComponent<EventTrigger>() != null;
		}
	}
	...
	private EventTrigger GetOrCreateEventTrigger {
		get {
			if (HasEventTrigger) {
				return sounds.GetComponent<EventTrigger>();
			}

			var trig = sounds.gameObject.AddComponent<EventTrigger>();
	...
	private void ActivateEvent(EventSounds.EventType eType) {
		switch (eType) {
			case EventSounds.EventType.UnitySliderChanged:
				sounds.useUnitySliderChangedSound = true;
				AddEventIfZero(sounds.unitySliderChangedSound);
				break;
			case EventSounds.EventType.UnityButtonClicked:
				sounds.useUnityButtonClickedSound = true;
				AddEventIfZero(sounds.unityButtonClickedSound);
				break;
			case EventSounds.EventType.UnityPointerDown:
				sounds.useUnityPointerDownSound = true;
				AddEventIfZero(sounds.unityPointerDownSound);
				break;
			case EventSounds.EventType.UnityPointerUp:
				sounds.useUnityPointerUpSound = true;
				AddEventIfZero(sounds.unityPointerUpSound);
				break;
			case EventSounds.EventType.UnityDrag:
				sounds.useUnityDragSound = true;
				AddEventIfZero(sounds.unityDragSound);
				break;
			case EventSounds.EventType.UnityDrop:
				sounds.useUnityDropSound = true;
				AddEventIfZero(sounds.unityDropSound);
				break;
			case EventSounds.EventType.UnityPointerEnter:
				sounds.useUnityPointerEnterSound = true;
				AddEventIfZero(sounds.unityPointerEnterSound);
				break;
			case EventSounds.EventType.UnityPointerExit:
				sounds.useUnityPointerExitSound = true;
				AddEventIfZero(sounds.unityPointerExitSound);
				break;
			case EventSounds.EventType.UnityScroll:
				sounds.useUnityScrollSound = true;
				AddEventIfZero(sounds.unityScrollSound);
				break;
			case EventSounds.EventType.UnityUpdateSelected:
				sounds.useUnityUpdateSelectedSound = true;
				AddEventIfZero(sounds.unityUpdateSelectedSound);
				break;
			case EventSounds.EventType.UnitySelect:
				sounds.useUnitySelectSound = true;
				AddEventIfZero(sounds.unitySelectSound);
				break;
			case EventSounds.EventType.UnityDeselect:
				sounds.useUnityDeselectSound = true;
				AddEventIfZero(sounds.unityDeselectSound);
				break;
			case EventSounds.EventType.UnityMove:
				sounds.useUnityMoveSound = true;
				AddEventIfZero(sounds.unityMoveSound);
				break;
			case EventSounds.EventType.UnityInitializePotentialDrag:
				sounds.useUnityInitializePotentialDragSound = true;
				AddEventIfZero(sounds.unityInitializePotentialDragSound);
				break;
			case EventSounds.EventType.UnityBeginDrag:
				sounds.useUnityBeginDragSound = true;
				AddEventIfZero(sounds.unityBeginDragSound);
				break;
			case EventSounds.EventType.UnityEndDrag:
				sounds.useUnityEndDragSound = true;
				AddEventIfZero(sounds.unityEndDragSound);
				break;
			case EventSounds.EventType.UnitySubmit:
				sounds.useUnitySubmitSound = true;
				AddEventIfZero(sounds.unitySubmitSound);
				break;
			case EventSounds.EventType.UnityCancel:
				sounds.useUnityCancelSound = true;
				AddEventIfZero(sounds.unityCancelSound);
				break;
		}
	}
	...
	private void DeactivateEvent(EventSounds.EventType eType) {
		switch (eType) {
			case EventSounds.EventType.UnitySliderChanged:
				sounds.useUnitySliderChangedSound = false;
				break;
			case EventSounds.EventType.UnityButtonClicked:
				sounds.useUnityButtonClickedSound = false;
				break;
			case EventSounds.EventType.UnityPointerDown:
				sounds.useUnityPointerDownSound = false;
				break;
			case EventSounds.EventType.UnityPointerUp:
				sounds.useUnityPointerUpSound = false;
				break;
			case EventSounds.EventType.UnityDrag:
				sounds.useUnityDragSound = false;
				break;
			case EventSounds.EventType.UnityDrop:
				sounds.useUnityDropSound = false;
				break;	
			case EventSounds.EventType.UnityPointerEnter:
				sounds.useUnityPointerEnterSound = false;
				break;
			case EventSounds.EventType.UnityPointerExit:
				sounds.useUnityPointerExitSound = false;
				break;
			case EventSounds.EventType.UnityScroll:
				sounds.useUnityScrollSound = false;
				break;
			case EventSounds.EventType.UnityUpdateSelected:
				sounds.useUnityUpdateSelectedSound = false;
				break;
			case EventSounds.EventType.UnitySelect:
				sounds.useUnitySelectSound = false;
				break;
			case EventSounds.EventType.UnityDeselect:
				sounds.useUnityDeselectSound = false;
				break;
			case EventSounds.EventType.UnityMove:
				sounds.useUnityMoveSound = false;
				break;
			case EventSounds.EventType.UnityInitializePotentialDrag:
				sounds.useUnityInitializePotentialDragSound = false;
				break;
			case EventSounds.EventType.UnityBeginDrag:
				sounds.useUnityBeginDragSound = false;
				break;
			case EventSounds.EventType.UnityEndDrag:
				sounds.useUnityEndDragSound = false;
				break;
			case EventSounds.EventType.UnitySubmit:
				sounds.useUnitySubmitSound = false;
				break;
			case EventSounds.EventType.UnityCancel:
				sounds.useUnityCancelSound = false;
				break;
		}
	}
                ...
                if (aEvent.isExpanded) {
                    ...
                    switch (aEvent.currentSoundFunctionType) {
                        case MasterAudio.EventSoundFunctionType.PlaySound:
                            ...
                            var newVol = DTGUIHelper.DisplayVolumeField(aEvent.volume, DTGUIHelper.VolumeFieldType.None, false, 0f, true);
						...
						case MasterAudio.EventSoundFunctionType.UnityMixerControl:
							...
							if (newMix != aEvent.currentMixerCommand) {
								UndoHelper.RecordObjectPropertyForUndo(ref isDirty, sounds, "change Unity Mixer Cmd");
							...
							switch (aEvent.currentMixerCommand) {
								case MasterAudio.UnityMixerCommand.TransitionToSnapshot:
									...
									if (newTime != aEvent.snapshotTransitionTime) {
										UndoHelper.RecordObjectPropertyForUndo(ref isDirty, sounds, "change Transition Time");
									...
									if (newSnap != aEvent.snapshotToTransitionTo) {
										UndoHelper.RecordObjectPropertyForUndo(ref isDirty, sounds, "change Snapshot");
								...
								case MasterAudio.UnityMixerCommand.TransitionToSnapshotBlend:
									...
									for (var i = 0; i < aEvent.snapshotsToBlend.Count; i++) {	
										...
										if (newWeight != aSnap.weight) {
											UndoHelper.RecordObjectPropertyForUndo(ref isDirty, sounds, "change Weight");
    ...
    private void SortCustomEventTriggers() {
        UndoHelper.RecordObjectPropertyForUndo(ref isDirty, sounds, "Alpha Sort Custom Event Triggers");

        sounds.userDefinedSounds.Sort(delegate(AudioEventGroup x, AudioEventGroup y) {
