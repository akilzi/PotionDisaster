...
public class MasterAudioInspector : Editor {
    ...
    public override void OnInspectorGUI() {
        ...
        var newMasterVol = DTGUIHelper.DisplayVolumeField(sounds.masterAudioVolume, DTGUIHelper.VolumeFieldType.None, false);
        ...
        if (plControllerInScene) {
            ...
            GUILayout.Space(5);
            var newPlaylistVol = DTGUIHelper.DisplayVolumeField(sounds.masterPlaylistVolume, DTGUIHelper.VolumeFieldType.None, false);
	            ...
	            if (sounds.useGroupTemplates) {
	                ...
	                if (Directory.Exists(groupTemplateFolder)) {
	                    ...
	                    if (grpTemplates > sounds.groupTemplates.Count) {
	                        ...
	                        DTGUIHelper.ShowLargeBarAlert("There are " + (grpTemplates - sounds.groupTemplates.Count) + " Group Template(s) that aren't set up in this MA prefab. Locate them");
				...
				if (filteredGroups.Count > 0) {
					...
					if (sounds.showGroupSelect) {
						...
						if (bulkSelectedGrps.Count > 0) {
							DTGUIHelper.ShowLargeBarAlert("Bulk editing - adjustments to a selected mixer Group will affect all " + bulkSelectedGrps.Count + " selected Groups.");
						} else {
							EditorGUILayout.Separator();
						}
					}
        ...
        if (sounds.playListExpanded) {
            ...
            if (!plControllerInScene) {
                ...
                for (var i = 0; i < pcs.Count; i++) {
                    ...
                    var newVol = DTGUIHelper.DisplayVolumeField(controller.playlistVolume, DTGUIHelper.VolumeFieldType.PlaylistController, false);
