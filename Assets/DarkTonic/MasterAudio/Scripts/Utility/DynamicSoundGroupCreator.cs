...
public class DynamicSoundGroupCreator : MonoBehaviour {
    ...
    public void CreateItems() {
        ...
        for (var i = 0; i < groupsToCreate.Count; i++) {
            ...
            var groupTrans = MasterAudio.CreateNewSoundGroup(aGroup, this.trans.name, errorOnDuplicates);
		...
		if (itemsCreatedEventExpanded) {
			MasterAudio.FireCustomEvent(itemsCreatedCustomEvent, this.trans.position);
		}
    }
