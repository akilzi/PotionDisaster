...
public class MasterAudio : MonoBehaviour {
	...
	public const string GroupTemplateFolder = MasterAudio.MasterAudioFolderPath + "/Sources/Prefabs/GroupTemplates/";
    public const string PREVIEW_TEXT = "*Fading & random settings are ignored by preview in edit mode.";
	public const float SEMI_TONE_PITCH_FACTOR = 1.05946f;
	public const float SPATIAL_BLEND_2D_VALUE = 0f;
	public const float SPATIAL_BLEND_3D_VALUE = 1f;
    ...
    void Awake() {
        ...
        if (!string.IsNullOrEmpty(firstGroupName)) {
			var result = PlaySound3DFollowTransform(firstGroupName, Trans, 0f, null, 0f, null);
    ...
    private static PlaySoundResult PlaySoundAtVolume(string sType,
        ...
        _group._playedForWarming = IsWarming;
        ...
        if (groupBus != null) {
            if (groupBus.BusVoiceLimitReached) {
                if (!groupBus.stopOldest) {
                    if (loggingEnabledForGrp) {
                        LogMessage("Bus voice limit has been reached. Cannot play the sound: " + _group._group.GameObjectName + " until one voice has stopped playing. You can turn on the 'Stop Oldest' option for the bus to change this.");
                    }
                    return null;
                } else {
        ...
        if (randomSource.variation.audLocation == AudioLocation.Clip && randomSource.variation.VarAudio.clip == null) {
            ...
            RemoveClipAndRefillIfEmpty(_group, isNonSpecific, randomIndex, choices, sType, pickedChoice, loggingEnabledForGrp);
        ...
        if (!soundSuccess) {
            ...
            if (_group._group.childGroupMode == MasterAudioGroup.ChildGroupMode.TriggerLinkedGroupsWhenPlayed && !triggeredAsChildGroup && !IsWarming) {
                for (var i = 0; i < _group._group.childSoundGroups.Count; i++) {
                    var childGrpName = _group._group.childSoundGroups[i];
                    PlaySoundAtVolume(childGrpName, volumePercentage, sourcePosition, pitch, sourceTrans, null, attachToSource, delaySoundTime, useVector3, false, false, false, true);
                }
    ...
    private static void SetLastPlayed(AudioGroupInfo _group) {
        _group._lastTimePlayed = Time.realtimeSinceStartup;
        _group._lastFramePlayed = Time.frameCount;
    }

    private static void RemoveClipAndRefillIfEmpty(AudioGroupInfo _group, bool isNonSpecific, int? randomIndex, List<int> choices, string sType, int pickedChoice, bool loggingEnabledForGrp) {
	...
	public static List<GameObject> GetAllPlayingVariationsInBus(string busName) {
        ...
        while (sources.MoveNext()) {
			...
			for (var i = 0; i < aInfo._sources.Count; i++) {
				var aVar = aInfo._sources[i].variation;
				if (!aVar.IsPlaying) {
					continue;
				}
    ...
    private static void SetBusVolume(GroupBus bus, float newVolume) {
        ...
        foreach (var key in audioSourcesBySoundType.Keys) {
            ...
            for (var i = 0; i < aGroup._sources.Count; i++) {
				...
				var grpVol = aGroup._group.groupMasterVolume * bus.volume * MasterAudio.Instance.masterAudioVolume;
